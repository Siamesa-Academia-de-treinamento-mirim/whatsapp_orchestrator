...
class Search_service
{
    ...
    public function search(string $query,array $types,int $limit=20,?int $conversationId=null):array
    {
        ...
        if($conversationId){$rows=$this->db->table('chat_messages')->select('id, conversation_id, message_type, text_content, sent_at, direction')->where('conversation_id',$conversationId)->where('deleted',0)->like('text_content',$query)->orderBy('message_timestamp','DESC')->limit($limit)->get()->getResultArray();foreach($rows as$r)$items[]=['type'=>'message','id'=>(int)$r['id'],'conversation_id'=>(int)$r['conversation_id'],'title'=>mb_substr((string)$r['text_content'],0,120),'subtitle'=>(string)$r['direction'].' · '.(string)$r['sent_at']];return['items'=>$items,'total'=>count($items)];}
